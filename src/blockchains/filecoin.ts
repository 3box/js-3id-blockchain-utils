...
export async function validateLink (proof: LinkProof): Promise<LinkProof | null> {
    const zondaxImport = process.env.JEST_WORKER_ID ? import("@zondax/filecoin-signing-tools/nodejs") : import("@zondax/filecoin-signing-tools")
    const signingTools = await zondaxImport
